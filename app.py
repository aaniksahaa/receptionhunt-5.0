...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:pAPa7XMh8qVaamypg3M1@containers-us-west-112.railway.app:6372/railway'
...
def puzzle():
    ...
    if request.method == 'POST':
        ...
        if current_level >= TOTAL_QUIZ:
            ...
            if is_location:
                is_correct = answer_lower == current_puzzle.answer.strip().lower()
            else:
                for puzzle_answer in puzzle_answers:
                    correct_answer = puzzle_answer.strip().lower().replace(' ', '')
                    print(correct_answer)
                    print(answer_lower)
                    if correct_answer == answer_lower:
                        is_correct = True
                        break
...
def admin():
    if request.method == 'GET':
        ...
        print(username, password)
        if username == "admin" and password == "AMRA_AMRAI_TO":
